...
if selected == "Signal Processing":
    ...
    if selected1 == 'Information':
        ...
        new_title = '<p style="font-family:Georgia; color:blue; font-size: 23px; text-align: left;">1.Bandpass Filter (IIR)Proses pertama adalah penerapan filter bandpass yang terdiri dari filter lowpass dan highpass. Filter ini berfungsi untuk menghilangkan noise dan komponen frekuensi yang tidak diinginkan dari sinyal ECG asli. Lowpass Filter berfungsi untuk menghilangkan komponen frekuensi tinggi. Sedangkan highpass Filter berfungsi untuk menghilangkan komponen frekuensi rendah.</p>'
        st.markdown(new_title, unsafe_allow_html=True)
        ...
        st.markdown(new_title, unsafe_allow_html=True)
        new_title = '<p style="font-family:Georgia; color:blue; font-size: 23px; text-align: left;">3.Squaring Operation Setelah diferensiasi, sinyal kemudian dikuadratkan. Operasi pengkuadratan ini memperkuat amplitudo sinyal dan memastikan bahwa semua nilai menjadi positif. Hal ini membantu dalam proses deteksi berikutnya dengan memperjelas puncak-puncak dalam sinyal.</p>'
        st.markdown(new_title, unsafe_allow_html=True)
        new_title = '<p style="font-family:Georgia; color:black; font-size: 20px; text-align: Justify;">4. Moving Average FilterLangkah terakhir adalah penerapan filter rata-rata bergerak. Filter ini menghaluskan sinyal dan membantu mengurangi noise yang tersisa setelah langkah-langkah sebelumnya. Filter rata-rata bergerak juga membantu dalam menyoroti tren utama dalam sinyal.</p>'
        st.markdown(new_title, unsafe_allow_html=True)
...
if selected == "HRV Analysis":
    ...
    if sub_selected == 'Time Domain Analysis':
        ...
        # Display heatmap in Streamlit
                ...
                def calculate_sd1(SDSD):
                    sd1 = math.sqrt(0.5 * SDSD**2)
                    return sd1
                
                sd1 = calculate_sd1(SDSD)
                st.write(f"SD1 : {sd1*1000}")
